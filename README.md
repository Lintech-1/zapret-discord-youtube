...
Выберите пункт **"6. Обновить файлы проекта"**. Обновление меняет только файлы в `$HOME/zapret-configs`.
...
Запустите `utils-zapret.sh` и выберите пункт **"7. Откатить последнее обновление"**.
...
3. **Обход не работает / перестал работать** — попробуйте следующие шаги:
   ...
   - Обновите IPSet через `utils-zapret.sh` → пункт 4 (обновить IPSet)
   - Проверьте режим IPSet через `utils-zapret.sh` → пункт 2 (переключить режим IPSet на `loaded`)
   - Запустите тесты через `utils-zapret.sh` → пункт 9 (запустить тесты) для диагностики
